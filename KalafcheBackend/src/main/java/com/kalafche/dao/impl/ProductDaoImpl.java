...
public class ProductDaoImpl extends JdbcDaoSupport implements ProductDao {
	...
	private static final String GET_ALL_PRODUCTS_QUERY = "select p.id, p.name, p.code, p.price, p.purchase_price, p.fabric, p.type_id, p.bonus_pts, p.sold_for_device_model_required, pt.name as type_name, pmt.id as master_type_id, pmt.name as master_type_name from product p left join product_type pt on p.type_id = pt.id left join product_master_type pmt on pmt.id = pt.master_type_id order by code desc";
	private static final String INSERT_PRODUCT = "insert into product(name, code, description, price, purchase_price, fabric, type_id, bonus_pts, sold_for_device_model_required) values (?, ?, ?, ?, ?, ?, ?, ?, ?)";
	private static final String UPDATE_PRODUCT = "update product set name = ?, description = ?, fabric = ?, price = ?, purchase_price = ?, type_id = ?, bonus_pts = ?, sold_for_device_model_required = ? where id = ?";
	...
	public void insertProduct(Product product) {
		getJdbcTemplate().update(INSERT_PRODUCT, product.getName(), product.getCode(), product.getDescription(),
				product.getPrice(), product.getPurchasePrice(), product.getFabric(), product.getTypeId(),
				product.getBonusPts(), Boolean.TRUE.equals(product.getSoldForDeviceModelRequired()));
	}
	
	@Override
	public void updateProduct(Product product) {
		getJdbcTemplate().update(UPDATE_PRODUCT, product.getName(), product.getDescription(), product.getFabric(),
				product.getPrice(), product.getPurchasePrice(), product.getTypeId(), product.getBonusPts(),
				Boolean.TRUE.equals(product.getSoldForDeviceModelRequired()), product.getId());
	}
